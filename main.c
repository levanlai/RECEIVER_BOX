...
#include "utils.h"
WORD cntPowerON=0;
WORD statusLedPowerON=LED_OFF;
WORD powerInit=FALSE;
...
void music_mode_sw(WORD mode)
{
	//select output : 0=SAM-DAAD2_QCC (QCC MUSIC) , 1=SMA-SPDIF-IN
    _orio(P0DIRPORT, 1 << 3);
    if(mode == 1)
    {
        _orio(P0VALPORT, 1 << 3);
