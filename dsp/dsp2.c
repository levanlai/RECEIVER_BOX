...
customPreInitFunction2( dspId );// Do all your custom pre initialization code into this function
	...
	_LiveMic_SetProcIN( dspId, BIQUAD_SAMPLE_IN|dsp2pcs[3], PCS_NODE | 8 );
	_LiveMic_SetProcOUT( dspId, BIQUAD_SAMPLE_OUT|dsp2pcs[3], PCS_NODE | 9 );

	// Process #4: LevelDetect
	dsp2pcs[4] = _LiveMic_LevelDetect_Allocate( dspId );
	_LiveMic_SetProcIN( dspId, LEVELDETECT_SAMPLE_IN|dsp2pcs[4], PCS_NODE | 9 );

	// Process #5: Compressor
	dsp2pcs[5] = _LiveMic_Compressor_Allocate( dspId );
	_LiveMic_SetProcIN( dspId, COMPRESSOR_SAMPLE_IN|dsp2pcs[5], PCS_NODE | 9 );
	_LiveMic_SetProcOUT( dspId, COMPRESSOR_SAMPLE_OUT|dsp2pcs[5], PCS_NODE | 6 );
	...
	_LiveMic_SetProcIN( dspId, MIXN_SAMPLE_IN|dsp2pcs[9], PCS_NODE | 0 );
