...
customPreInitFunction1( dspId );// Do all your custom pre initialization code into this function
	...
	// Process #2: Gain
	dsp1pcs[2] = _LiveMic_Gain_Allocate( dspId );
	_LiveMic_SetProcIN( dspId, GAIN_SAMPLE_IN|dsp1pcs[2], PCS_DSP_IN | 0 );
	_LiveMic_SetProcOUT( dspId, GAIN_SAMPLE_OUT|dsp1pcs[2], PCS_NODE | 7 );

	// Process #3: NoiseGate
	dsp1pcs[3] = _LiveMic_NoiseGate_Allocate( dspId );
	_LiveMic_SetProcIN( dspId, NOISEGATE_SAMPLE_IN|dsp1pcs[3], PCS_NODE | 7 );
	_LiveMic_SetProcOUT( dspId, NOISEGATE_SAMPLE_OUT|dsp1pcs[3], PCS_NODE | 8 );

	// Process #4: FreqShifter
	dsp1pcs[4] = _LiveMic_FreqShifter_Allocate( dspId );
	_LiveMic_SetProcIN( dspId, FREQSHIFTER_SAMPLE_IN|dsp1pcs[4], PCS_NODE | 8 );
	_LiveMic_SetProcOUT( dspId, FREQSHIFTER_SAMPLE_OUT|dsp1pcs[4], PCS_NODE | 9 );

	// Process #8: Gain
	dsp1pcs[8] = _LiveMic_Gain_Allocate( dspId );
	_LiveMic_SetProcIN( dspId, GAIN_SAMPLE_IN|dsp1pcs[8], PCS_NODE | 9 );
	_LiveMic_SetProcOUT( dspId, GAIN_SAMPLE_OUT|dsp1pcs[8], PCS_NODE | 6 );
	...
	// Process #6: MixN
	dsp1pcs[6] = _LiveMic_MixN_Allocate( dspId, 2 );
	_LiveMic_SetProcIN( dspId, MIXN_SAMPLE_IN|dsp1pcs[6], PCS_NODE | 0 );
	_LiveMic_SetProcOUT( dspId, MIXN_SAMPLE_OUT|dsp1pcs[6], PCS_NODE | 5 );

	// Process #5: MixN
	dsp1pcs[5] = _LiveMic_MixN_Allocate( dspId, 2 );
	_LiveMic_SetProcIN( dspId, MIXN_SAMPLE_IN|dsp1pcs[5], PCS_NODE | 2 );
	_LiveMic_SetProcOUT( dspId, MIXN_SAMPLE_OUT|dsp1pcs[5], PCS_NODE | 4 );

	// Process #7: MixN
	dsp1pcs[7] = _LiveMic_MixN_Allocate( dspId, 3 );
	_LiveMic_SetProcIN( dspId, MIXN_SAMPLE_IN|dsp1pcs[7], PCS_NODE | 4 );
	_LiveMic_SetProcOUT( dspId, MIXN_SAMPLE_OUT|dsp1pcs[7], PCS_DSP_OUT | 0 );
...
const WORD nrpn1List[NUMBEROFCOMMAND1][2]=
{
	...
	{ 0x0300, 0x0038 }, // _LiveMic_NoiseGate_OnOff
	{ 0x0301, 0x0039 }, // _LiveMic_NoiseGate_Threshold
	{ 0x0302, 0x003A }, // _LiveMic_NoiseGate_Attack
	{ 0x0303, 0x003B }, // _LiveMic_NoiseGate_Release
	{ 0x0304, 0x003C }, // _LiveMic_NoiseGate_InGainValue
	{ 0x0305, 0x003D }, // _LiveMic_NoiseGate_InGainPhase
	{ 0x0400, 0x002A }, // _LiveMic_FreqShifter_OnOff
	{ 0x0401, 0x002B }, // _LiveMic_FreqShifter_PostHP
	{ 0x0402, 0x002C }, // _LiveMic_FreqShifter_Amount
	{ 0x0403, 0x002D }, // _LiveMic_FreqShifter_SetMaxScale
	{ 0x0404, 0x002E }, // _LiveMic_FreqShifter_InGainValue
	{ 0x0405, 0x002F }, // _LiveMic_FreqShifter_InGainPhase
	...
	{ 0x0800, 0x0030 }, // _LiveMic_Gain_Value
	{ 0x0801, 0x0031 } // _LiveMic_Gain_Phase
